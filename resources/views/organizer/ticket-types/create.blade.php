...
@section('title', 'Crear Tipo de Entrada')

@section('content')
<div class="max-w-lg mx-auto">
    <h1 class="text-2xl font-bold text-gray-800 mb-2">Nuevo tipo de entrada</h1>
    ...
    <form method="POST" action="{{ route('organizer.events.ticket-types.store', $event) }}" class="bg-white rounded-xl shadow p-6 space-y-4">
        @csrf
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Nombre *</label>
            <input type="text" name="name" value="{{ old('name') }}" required placeholder="Ej: General, VIP"
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="datetime-local" name="sale_start" value="{{ old('sale_start') }}"
                    class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Fin de venta</label>
                <input type="datetime-local" name="sale_end" value="{{ old('sale_end') }}"
                    class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
            </div>
        </div>
        <div>
        ...
        <div class="flex gap-3">
            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-lg font-medium transition">Crear</button>
